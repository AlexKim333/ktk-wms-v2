...
for file in files:
    ...
    content = re.sub(r'alert\(\[즉배요청 성공\].*?\)', 'alert([즉배요청 성공] 즉시 출고 전표()가 대기열(Draft)에 성공적으로 생성되었습니다!)', content)
    content = re.sub(r'alert\(\[예약 성공\].*?\)', 'alert([예약 성공] 예약 요청()이 보류(Pending) 상태로 성공적으로 전송되었습니다!)', content)
